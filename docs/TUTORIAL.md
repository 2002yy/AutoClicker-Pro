...
1. **录制一个点击**：按 `F10`（或点"开始录制"）→ 在目标位置点几下鼠标 → 按 `ESC` 停止
2. **设定循环**：重复次数填 `10`、轮次间隔填 `1000`（毫秒，即每秒一轮）
3. **开跑**：按 `F8` 启动；再按 `F8` 或 `ESC` 随时停止

![录制与回放](screenshots/record-replay.png)
*（截图占位：动作列表里出现"按下/释放"条目的状态）*
...
| 项目 | 含义 |
|---|---|
| 点击间隔 | 序列内相邻动作的间隔（仅对无时间戳的序列生效；录制的序列按原速回放） |
| 按住持续时间 | 单独按下动作的按压时长 |
| 重复次数 | 整个序列播放几轮 |
| 轮次间隔 | 两轮之间的等待（毫秒） |
